/**
 * IPC Contracts for Workflow Execution Popup
 *
...
/**
 * IPC channels for workflow popup
 *
 * Usage in main process:
 *   ipcMain.handle('workflow-popup:analyze', handler)
 *
 * Usage in renderer (via preload):
 *   window.electron.workflowPopup.analyze(workflowId)
 */
export interface WorkflowPopupIPC {
  /**
   ...
   */
  'workflow-popup:analyze': (workflowId: string) => Promise<WorkflowAnalysisResult>;

  /**
   * Get stored popup configuration for a workflow
   * @param workflowId - The n8n workflow ID
   * @returns Stored config or null if not found
   */
  'workflow-popup:get-config': (workflowId: string) => Promise<WorkflowPopupConfig | null>;
   ...
   */
  'workflow-popup:save-config': (config: WorkflowPopupConfig) => Promise<{ success: boolean }>;
   ...
   * @returns Success status
   */
  'workflow-popup:delete-config': (workflowId: string) => Promise<{ success: boolean }>;

  /**
   * Execute a workflow with provided inputs
   * @param request - Execution request with inputs
   * @returns Execution start response with ID
   */
  'workflow-popup:execute': (request: ExecuteWorkflowRequest) => Promise<ExecuteWorkflowResponse>;
   ...
   */
  'workflow-popup:status': (executionId: string) => Promise<ExecutionStatusResponse>;
   ...
   * @returns Success status
   */
  'workflow-popup:cancel': (executionId: string) => Promise<{ success: boolean }>;
   ...
   */
  'workflow-popup:select-files': (options: FileSelectOptions) => Promise<FileSelectResult>;
   ...
   * @returns Save path
   */
  'workflow-popup:save-file': (options: FileSaveOptions) => Promise<FileSaveResult>;

  /**
   * Copy file from workflow output to user location
   * @param sourcePath - Source file path (from workflow output)
   * @param destinationPath - User-selected destination
   * @returns Success status
   */
